...
続けて、各toolの`self-test`（正例・反例・境界例: 空stdin、不正JSON、入力資料のpathの欠落、旧引数のargparse拒否）と、SKILL.mdの手順と同じ形でfixtureを渡す実行を行う。条件マトリクス、domainのGherkin検査、誰が行えるかの網羅、ユーザー目的達成BDD、formulation 3入口の同一パス更新を検査する。RDB物理設計の検査は独立した`rdb-design` packageが所有する。
