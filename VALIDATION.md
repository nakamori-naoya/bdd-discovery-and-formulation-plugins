...
点数には目安の帯を置く。85点以上は、下流の資料へそのまま渡せる「実用に足る」、70点以上85点未満は、減点の条件を直せば使える「手直しで使える」、70点未満は「作り直しが要る」である。帯は合否ではない。報告には、点数と一緒に、減点の大きかった条件と採点役の根拠を並べる。採点の報告は `evals/results/grading/<ケース>-<時刻>.md` に、各回の判定は同じ名前の `.vote<N>.md` に残る。
