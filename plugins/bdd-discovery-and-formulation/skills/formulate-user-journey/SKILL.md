...
description: 既存のユーザー目的達成BDDを、目的、両端、場面接続、分岐、中断再開、役割移譲、完了の観測可能性から反証し、確認済みの理解と未決を同じ正式な定義へ戻す。「このUser Journey BDDを深掘りして」「目的達成シナリオの抜けを検査して」と言われたときに使う。新しい資料、ユースケース、ドメインルール、データモデル、テスト仕様は作らない。
---
