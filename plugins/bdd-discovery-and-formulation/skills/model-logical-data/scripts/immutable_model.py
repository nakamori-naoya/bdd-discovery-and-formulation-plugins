...
判定のよりどころ: この入口の SKILL.md が書くイミュータブルデータモデルの型（三つのテーブル、命名、時刻は occurred_at の一本）と
  references/technical-process-lifecycle.md（技術処理の命名と時刻）。記法は write-doc の rdb-logical-data-modeling 型の「検査が読む目印」。見出しの文言は読まない。
