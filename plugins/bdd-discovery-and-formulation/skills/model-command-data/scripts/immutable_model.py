...
def parse_references(label: str, prose: list[tuple[int, str]], problems: list[Problem]) -> dict[str, tuple[list[str], str]]:
    ...
    for line_number, cells in (tables[0] if tables else []):
        ...
        if not match or not COLUMN_LIST.fullmatch(columns) or not LINK.fullmatch(owner):
        ...
        if match.group(1) in references:
            ...
            continue
        references[match.group(1)] = (re.findall(r"`([^`]+)`", columns), LINK.fullmatch(owner).group(1))
    return references
...
def check_model(label: str, model: Model, problems: list[Problem]) -> None:
    ...
    for name in sorted(referenced - drawn):
