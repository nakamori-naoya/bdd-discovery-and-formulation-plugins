#!/usr/bin/env python3
"""formulationの更新先が、入力された既存の資料と同じ実体であることを検査する。

  update-guard.py check --existing <既存の資料の絶対path> --output <更新先として決めた絶対path>
      -> exit 0 で {"update_target": <解決済み絶対path>} をstdoutへ返す
         exit 1 = 既存の資料がsymlinkか通常fileでない、または両pathが別の実体を指す（stdoutへ {"error": ...}）
...
意味評価として残す範囲: その資料が本当に更新対象として妥当か、更新内容が資料の型に合うか。
"""
...
def guard(existing_path, output_path):
    ...
    if existing.is_symlink() or not existing.is_file():
        return "入力はsymlinkではない既存の資料のfileでなければならない: {}".format(existing_path), None
    if existing.resolve() != output.resolve():
        return "formulationは新規資料を作らず、入力された既存の資料と同じpathを更新する: existing={} output={}".format(existing_path, output_path), None
...
def self_test():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert guard(link, link)[0], "反例: symlinkの既存の資料"
        assert guard(Path(tmp) / "missing.md", Path(tmp) / "missing.md")[0], "反例: 存在しない既存の資料"
