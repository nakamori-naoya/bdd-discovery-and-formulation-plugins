...
description: 資料の有無にかかわらず、1人の主たるユーザーが1つの目的を達成するまでを発見し、開始地点、完了条件、接続した場面のBDDを新しい正本として1本作る。「ユーザーJourneyをBDDで発見して」「目的達成までの未知の振る舞いを探索して」と言われたときに使う。指定した既存正本の反証と同じpathへの更新、ドメインルール、データモデル、テスト実行は扱わない。
---

# ユーザー目的達成BDDの新しい正本を作る
...
**保存先の決め方。** 依頼に資料構成が示されていれば、そのまま使う。示されていなければ、利用者の既存資料のdirectory階層と命名を読み、主たるユーザーごとに分ける置き場と名前を提案として`settle`の問いの1つに含めて確かめる。正本の置き場は利用者の資料構成に属するので、仮置きで書かない。既定のpathを持たない。`output_directory`が存在しないか相対pathなら止まって利用者へ返す。出力先と同じpathのfileがすでにある場合は上書きせず、未使用の名前を`settle`で提案して確かめる。同じpathへ新規保存できないことを、既存資料があるため探索できないことや、formulationへ切り替える理由にしない。未使用の保存先を確認できれば、既存資料を根拠として探索を続ける。
...
| 観察対象 | 述語 | 行動 |
|---|---|---|
| 同種または関連する既存資料 | 未知の目的、両端、場面、状態の受け渡しを探索して、新しいJourney正本を作る依頼である | 既存Journey、業務知識、画面案を入力根拠として読み、記載済みというだけで確認済みにせずDiscoveryを続ける。指定された既存Journey正本を反証し、結果を同じpathへ戻す依頼だけをformulationとする |
