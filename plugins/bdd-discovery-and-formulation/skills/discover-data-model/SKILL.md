...
| 観察対象 | 述語 | 行動 |
|---|---|---|
| 業務知識のシナリオ | 正常系、または論理制約・トランザクション・履歴・読み取り投影が結果を左右する | 永続化シナリオへ採用する。認可や永続化前の入力受付だけで拒否が決まるものは業務知識に残し、採否表へ対象外理由を書く |
| 同種または関連する既存資料 | 未知の永続化理由、残す事実、履歴、取消・訂正を探索して、新しい論理設計を作る依頼である | 既存の論理設計、業務知識、Journeyを入力根拠として読み、記載済みというだけで確認済みにせずDiscoveryを続ける。指定された既存論理設計を反証し、結果を同じpathへ戻す依頼だけをformulationとする |
...
| 分類表の`正式な定義`列 | 業務知識に、複数の意味ある業務イベントと、その順序・履歴・取消・訂正を後から説明する必要がある | 保存表現としてイベント列を候補にする。出来事が無いか乏しく履歴を使わないなら現在状態、設定変更の履歴自体を扱うなら有効期間履歴を候補にし、選択理由を資料へ残す。イベント列を選ぶ場合はevent recordを状態再構築の一次データ、現在状態や検索用投影をその参照元から作る派生物として区別する |
| 出来事の性質 | 顧客・業務担当者が意味を説明できる業務イベントである | 業務イベントとして扱う。業務イベントを外へ知らせる、要求を回収して送る、再試行する、調停するといった技術的な処理は、業務イベントと分けて技術処理のライフサイクルとして設計する。その根拠は業務知識ではなく、依頼か`references`の要求資料に置く |
