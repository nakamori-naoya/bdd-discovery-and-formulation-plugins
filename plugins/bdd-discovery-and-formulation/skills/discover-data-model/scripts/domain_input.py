...
def validate_file(value: object, path: str, problems: list[dict[str, str]]) -> str | None:
...
def validate_paths(values: object, path: str, problems: list[dict[str, str]]) -> list[str]:
    if not isinstance(values, list) or not values:
        problems.append(diagnostic(path, "1件以上のlistではない", "対応する業務知識の絶対pathを1件以上指定する"))
        return []
    normalized: list[str] = []
    for index, value in enumerate(values):
        result = validate_file(value, f"{path}[{index}]", problems)
...
def check(payload: object) -> tuple[list[dict[str, str]], dict[str, object]]:
    ...
    if "targets" in payload:
        ...
        for index, target in enumerate(targets):
            ...
            if not isinstance(target, dict) or set(target) != {"logical_document_path", "business_knowledge_paths"}:
                ...
                continue
            logical = validate_file(target["logical_document_path"], f"{base}.logical_document_path", problems)
            knowledge = validate_paths(target["business_knowledge_paths"], f"{base}.business_knowledge_paths", problems)
    ...
    knowledge = validate_paths(payload["business_knowledge_paths"], "business_knowledge_paths", problems)
