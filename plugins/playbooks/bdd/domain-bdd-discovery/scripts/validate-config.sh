...
jq -e '
  ...
  [.steps[].id]==["settle","ground","explore","scope","record-behavior","validate","document"] and
  [.steps[] | (.agent_work // .script // .playbook)]==["grill","invoking_agent","invoking_agent","invoking_agent","invoking_agent","scripts/scenario_matrix.py","write-doc"] and
  ...
  .steps[-1].needs==["final_markdown","validation_report","output_directory","name"] and (.steps[-1].provides|index("domain_rule_path")!=null)
' "$file" >/dev/null || { echo "[error] domain-bdd-discoveryはYAML宣言順、同一agentの認知工程、決定論的検査、text資料化の構造契約を満たすこと" >&2; exit 2; }
