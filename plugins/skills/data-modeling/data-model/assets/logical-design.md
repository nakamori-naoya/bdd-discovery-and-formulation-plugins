...
<!-- 執筆指示: 各節の「書く/書かない」を満たしてから、このコメントを削除する。
     ...
     0件なら「レコードなし」、変化しないなら「変更なし」と明記する。
...
<!-- 書く: 全論理テーブル、全論理列、PK・FK・UK、型、NOT NULL、値域、多重度。書かない: 対象DBMS固有の型やindex。 -->

<!-- 英語名（日本語名） : PK・FK等 / 型 / NULL制約 / 値域または説明、の順で書く。
     日本語名を引用符で重複させない。図のテーブルと列は後述する定義と一致させる。 -->

```mermaid
classDiagram
    class logical_table_a {
        id（番号） : PK / uuid / NOT NULL
        status（状態） : text / NOT NULL / <取り得る値>
    }
    class logical_table_b {
        logical_table_a_id（A番号） : PK・FK / uuid / NOT NULL
    }
    logical_table_a "1" --> "0..1" logical_table_b : 業務上の関係
```
