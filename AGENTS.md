...
- `write-doc`と`grill`は同梱せず、別repositoryへは`playbook:`工程だけで依存する。相手を`skill:`や`script:`で指さない。相手の中の部品名、工程の呼び名、保存の呼び名、script、参考資料、設定へ触れず、依存versionも固定しない。呼び方は`references/nested-playbook.md`の公開契約に従う。どちらも公開入口へ入力objectを直接渡し、相手の設定解決は行わない。`write-doc/write-doc`版2と`grill/grill`版1が直接返す結果objectを、旧YAML relayと混同しない。
