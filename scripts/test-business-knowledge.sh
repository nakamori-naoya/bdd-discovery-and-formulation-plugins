...
fresh; edit "$user" 's#\.\./予約/business-knowledge\.md#../無い/business-knowledge.md#'; rejects "実在しない持ち主を拒否" "持ち主の資料を読めない" "$user"
fresh; edit "$user" 's/\| 予約 \| Reservation \| 業務用語 \| \[[^\n]*/| 予約 | Reservation | 業務用語 | この資料 |/'; rejects "二本の資料が同じ言葉を決める組を拒否" "同じ言葉を二本の資料が決めている" "$owner" "$user"
fresh; edit "$user" 's/\| 入室した \| Entered \|/| 入室した | Reserved |/'; rejects "同じ英名が違う言葉に付く組を拒否" "英名 Reserved が別の言葉" "$owner" "$user"
